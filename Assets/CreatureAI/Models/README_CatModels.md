...
## 小物モデル（Prop_Churu / Prop_NekoJarashi）

ポイント用の小物です（テクスチャ不要・マテリアル色のみ、猫よりずっと小さい実寸）。

| ファイル | 内容 | サイズ | 使い方 |
|---|---|---|---|
| `Prop_Churu.fbx` | チュール風おやつ（先端からペースト） | 全長 約13cm | FoodBowl（餌ポイント）の見た目に |
| `Prop_NekoJarashi.fbx` | 猫じゃらし（棒＋鈴＋ふさふさ） | 全長 約24cm | Toy（おもちゃポイント）の見た目に |

**差し替え方**: 各ポイント（FoodBowl / Toy など）の子にある `Mesh`（円柱などの仮の見た目）を
削除し、代わりにこのFBXをポイントの子としてドラッグするだけ。ポイントの機能
（CreaturePoint）はルート側にあるので、見た目を替えても動作は変わりません。
どちらも原点が接地面の中央にあり、床にそのまま置ける向きで入っています。
